...
def main():
    ...
    if os.path.exists(csv_clean_path) and os.path.exists(csv_messy_path):
        ...
        print("1. Extracting parameters from real-world data...")
    ...
    def train_and_eval(data, model, label=""):
        X = data[features]
    ...
    for name, clf in models.items():
        print(f"--- {name} ---")
        train_and_eval(df_clean_baseline, clf, "Clean Synthetic")
        _, _, _, _, auc_m = train_and_eval(df_imputed, clf, "Messy -> Imputed")
        # For plotting, we save the Random Forest results
        if name == "Random Forest":
            rf_messy = clf
            rf_clean, X_test_c, y_test_c, prob_c, auc_c = train_and_eval(df_clean_baseline, clf, "Clean (saved for plot)")
            rf_m, X_test_m, y_test_m, prob_m, _ = train_and_eval(df_imputed, clf, "Messy (saved for plot)")
        print()

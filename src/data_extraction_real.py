...
def load_real_historical_data(n_patients: int = 500, seed: int = 101) -> pd.DataFrame:
    """
    Loads real historical datasets (HDHI Admissions & CKD Nephrotoxic Drug) 
    to form the baseline clinical cohort instead of relying on synthetic generation.
    """
    import os
    np.random.seed(seed)

    # 1. Load Real HDHI Admission Data for Demographics & Baseline Labs
    hdhi_path = os.path.join("data", "datasets_not_organized", "useful_for_our_case", "HDHI Admission data.csv")
    if not os.path.exists(hdhi_path):
        raise FileNotFoundError(f"Could not find real HDHI dataset at {hdhi_path}")
    
    hdhi = pd.read_csv(hdhi_path)

    # Map the real data to our expected columns
    df_base = pd.DataFrame()
    df_base['patient_id'] = [f"HIST_{i:04d}" for i in range(len(hdhi))]
    
    # Age and Gender
    df_base['age'] = pd.to_numeric(hdhi['AGE'], errors='coerce').fillna(56.0).clip(18, 95).astype(int)
    # Convert 'M'/'MALE' to 1, 'F'/'FEMALE' to 0
    df_base['gender'] = hdhi['GENDER'].astype(str).str.strip().str.upper().str.startswith('M').astype(int)
    
    # Comorbidities (HDHI uses 1/0 or Yes/No, assuming 1/0 based on standard, we coerce)
    df_base['has_htn'] = pd.to_numeric(hdhi['HTN'], errors='coerce').fillna(0).astype(int)
    df_base['has_dm'] = pd.to_numeric(hdhi['DM'], errors='coerce').fillna(0).astype(int)
    
    # Baseline Serum Creatinine
    df_base['baseline_scr'] = pd.to_numeric(hdhi['CREATININE'], errors='coerce').fillna(1.0).clip(0.2, 15.0)

    # 2. Extract Vancomycin Toxicity Parameter from Real Drug Dataset
    drug_path = os.path.join("data", "datasets_not_organized", "useful_for_our_case", "CKD_NephrotoxicDrug_Dataset.csv")
    vanco_toxic_boost = 0.31  # Default fallback
    if os.path.exists(drug_path):
        drug_df = pd.read_csv(drug_path)
        vanco_patients = drug_df[drug_df['drug_name'].str.contains('Vancomycin', case=False, na=False)]
        if len(vanco_patients) > 0:
            vanco_toxic_boost = len(vanco_patients[vanco_patients['serum_creatinine_change_pct'] > 15.0]) / len(vanco_patients)

    # 3. Simulate Drug Exposures (since HDHI doesn't track Vanco/Zosyn explicitly)
    ...
    df_base['received_zosyn'] = np.random.binomial(1, 0.42, len(df_base))

    # 4. Hybrid Outcomes (Real Baseline AKI + Drug Toxicity Synergy)
    real_base_aki = pd.to_numeric(hdhi['AKI'], errors='coerce').fillna(0).astype(int)
    
    aki_prob = []
    for idx, row in df_base.iterrows():
        p = float(real_base_aki.iloc[idx]) * 0.4
        if p == 0.0:
            p = 0.07  # Population baseline risk
        ...
        if row['received_vanco'] == 1 and row['received_zosyn'] == 1:
            p += 0.11  # Synergistic boost
            
        aki_prob.append(min(0.95, p))

    df_base['developed_aki'] = np.random.binomial(1, aki_prob)

    # 5. Sample down to requested n_patients
    if n_patients < len(df_base):
        df_base = df_base.sample(n=n_patients, random_state=seed).reset_index(drop=True)
